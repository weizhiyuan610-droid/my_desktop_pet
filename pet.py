...
class DesktopPet(QWidget):
    def __init__(self):
        ...
        # 初始配置
        self.normal_size = 250  # 正常大小（像素，宽度）
    ...
    def load_cat_image(self):
        ...
        if pixmap.isNull():
            ...
            print("   请确保 cat.png 是有效的 PNG/JPG 图片")
            print("\n程序无法启动，请修复图片文件后重试。")
            sys.exit(1)  # 直接退出程序
        ...
        # 计算窗口尺寸
        # 宽度固定为 250，高度根据图片比例计算
        target_width = 250
        target_height = int(250 * pixmap.height() / pixmap.width())
    ...
    def update_window_snapping(self):
        ...
        try:
            ...
            # 计算猫咪位置
            # X 轴：向内偏移 10px，避免悬空感
            pet_x = window_x + window_width - pet_width - 10

            # Y 轴：考虑标题栏高度，让猫咪趴在视觉可见的顶端
            # 基础位置 + 标题栏高度修正 - 猫咪高度 + 20px 压住边缘
            pet_y = window_top_qt - pet_height + titlebar_height + 20

            # App 特殊处理：全屏或自定义 UI 应用
            if app_name in ['人人视频', '腾讯视频', '爱奇艺', '优酷', 'VLC', 'QuickTime Player']:
                # 全屏应用可能没有标准标题栏，增加额外的负偏移
                pet_y -= 15
